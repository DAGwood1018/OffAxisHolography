...
class AlignOffAxis(OffAxisMask):
    ...
    def __call__(self, threads=1, flags=0):
        phase_extract = ExtractPhase(np.array(self._dims, dtype=int), np.array(self.roi, dtype=int), self._mask.astype(np.double), self._ref.astype(np.complex128), threads, flags)
