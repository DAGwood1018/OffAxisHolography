 ...
 * TODO Cropping is not working properly
**/
...
void OffAxFilter::show(fftw_complex *arr, bool cropped, bool log_scale) {
    int N;
    int x;
    int y;
    if (cropped) {
...
void OffAxFilter::show_input() {
    this->show(this->a, false, false);
}


void OffAxFilter::show_output() {
    this->show(this->d, true, false);
}


void OffAxFilter::show_filtered() {
    this->show(this->c, true, true);
}
